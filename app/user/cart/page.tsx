...
export default async function CartPage() {
  ...
  const cartItems = result.success && result.data ? result.data : [];

  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-background text-foreground">
      <div className="w-full max-w-2xl p-8 flex flex-col items-center">
        <h1 className="text-3xl font-bold mb-2">Shopping Cart</h1>
        <p className="mb-8 text-muted-foreground">Review your items before checkout</p>
        {cartItems.length === 0 ? (
          <Card className="p-12 flex flex-col items-center w-full">
            <CardContent className="flex flex-col items-center">
              ...
              <span className="mb-4 text-muted-foreground">Start shopping to add items to your cart</span>
